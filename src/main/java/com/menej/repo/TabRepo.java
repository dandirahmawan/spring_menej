...
public interface TabRepo extends JpaRepository<Tab, Integer> {
    String sql = "SELECT new com.menej.model.DataTab (\n" +
                        "t.tabId, \n" +
                        "t.projectId, \n" +
                        "t.tabName, \n" +
                        "t.createdBy, \n" +
                    ...
                    "LEFT JOIN UserPrivacyTab userPrivacy ON userPrivacy.tabId = t.tabId \n"+
                    "WHERE \n" +
...
//                    "OR (t.projectId = ?1 AND p.pic = ?2 AND (t.isDelete IS NULL OR t.isDelete != 'Y')) \n" +
    ...
    List<Tab> findByTabIdAndProjectId(int tabId, int projectId);
